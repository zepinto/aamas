...
/**
 * GEPIndividual is an Individual used for GEP evolution runs. It encodes a
...
public class GEPIndividual extends Individual
{
    ...
    /** Each individual (or genome) can have one or more genes, so genome 
	 ...
	 * must have their results converted to a 0 or a 1. The threshold is used to 
    ...
    public void setup(final EvolutionState state, final Parameter base)
    {
        ...
        // set my evaluation to false and clear the parsedGeneExpressions
        evaluated = false;
        ...
        try {
			...
			{   s = s + "Gene " + i + "\n";
			    ...
			    {	String thePrintableSymbol;
			        GEPSymbol sym = ((GEPSpecies)this.species).symbolSet.symbols[genome[i][j]];
			    ...
			    if (((GEPSpecies)species).useConstants)
			    {   s = s + "\n";
    ...
    {
    	boolean hasConstants = ((GEPSpecies)species).useConstants;
    	...
    	{   // parse each gene into an expression tree
    		if (hasConstants)
        		parsedGeneExpressions[i] = parseGeneWithConstants(genome[i], genomeDc[i], 
        				genomeConstants[i], ((GEPSpecies)species).symbolSet);
    		else
    		    parsedGeneExpressions[i] = parseGene(genome[i], ((GEPSpecies)species).symbolSet);
    	}
    }
     ...
     * problem is initialized (from a file with the test data or by asking the user problem
    ...
    {
        int counts[] = new int[((GEPSpecies)(((Individual)this).species)).symbolSet.numberOfTerminals];
        // if parsedGeneExpressions is null then must parse the expression before
        // we can determine its size
        if (parsedGeneExpressions == null)
        	parseGenes();
